...
export default function CardWarsGame({ sessionId }: { sessionId: string }) {
  ...
  const myRoundXp = reveal?.roundXp[mySlot] ?? 0;
  const oppRoundXp = reveal?.roundXp[oppSlot] ?? 0;
  const roundWinner: "me" | "opp" | "tie" | null = reveal
    ? myRoundXp > oppRoundXp
      ? "me"
      : myRoundXp < oppRoundXp
        ? "opp"
        : "tie"
    ...
    <div className="relative h-svh w-full overflow-hidden bg-[#0f100f]">
      ...
      <main className="absolute inset-0 z-10">
        <ScoreLabel
          label="OPPONENT"
          score={oppXp}
          className="left-4 top-[134px] items-start sm:left-[calc(50%-234px)] sm:top-[97px]"
        />
        <ScoreLabel
          label="YOU"
          score={myXp}
          className="right-4 top-[234px] items-end sm:right-[calc(50%-260px)] sm:top-[216px]"
        />

        <PlayerSlot
          state={slotState("opp")}
          className="left-[23px] top-[205px] sm:left-[calc(50%-211px)] sm:top-[174px]"
          tilt="-rotate-6"
          highlight={evaluated ? (roundWinner === "opp" ? "win" : "lose") : null}
        />
        <PlayerSlot
          state={slotState("me")}
          className="right-[8px] top-[295px] sm:right-[calc(50%-214px)] sm:top-[287px]"
          tilt="rotate-6"
          highlight={evaluated ? (roundWinner === "me" ? "win" : "lose") : null}
        />
  ...
  return (
    <>
      ...
      <img
        ...
        draggable={false}
      />
      <div className="pointer-events-none absolute left-1/2 top-1/2 hidden aspect-1280/832 w-full max-w-[1280px] -translate-x-1/2 -translate-y-1/2 opacity-70 mix-blend-screen sm:block">
        <img
      ...
      <img
        ...
        alt=""
        aria-hidden
        className="pointer-events-none absolute inset-0 block h-full w-full object-cover opacity-70 mix-blend-screen sm:hidden"
        draggable={false}
      />
    </>
  );
...
function ScoreLabel({
  ...
  return (
    ...
    >
      ...
      <span className="mt-1 text-[20px] uppercase text-white/80">{label}</span>
    </div>
  );
}
